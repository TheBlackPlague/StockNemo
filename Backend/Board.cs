...
public class Board
{
    ...
    public RevertMove Move<MoveType>(Square from, Square to, Promotion promotion = Promotion.None)
        where MoveType : MoveUpdateType
    {
        ...
        // Make the move.
        Map.Move<MoveType>(pieceF, colorF, pieceT, colorT, from, to);
    ...
    [MethodImpl(MethodImplOptions.AggressiveOptimization)]
    public void UndoMove<MoveType>(ref RevertMove rv) where MoveType : MoveUpdateType
    {
        // Remove castling rights from hash to allow easy update.
        Zobrist.HashCastlingRights(
            ref Map.ZobristHash, 
        ...
        Map.Move<MoveType>(pF, cF, pT, cT, rv.To, rv.From);
        ...
        if (rv.SecondaryFrom != Square.Na) Map.Move<MoveType>(rv.SecondaryTo, rv.SecondaryFrom);
    }
